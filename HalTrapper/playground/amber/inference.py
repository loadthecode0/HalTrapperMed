...
def main(args):
    ...
    # HalTrapper: Added
    debug_output = []
    debug_output_path = args.inference_data + "-amber-debug.json"
    # Addition Ends
    ...
    for i in tqdm(range(len(inference_data))):
        ...
        if ground_truth[id - 1]["type"] == "generative":
            ...
            for n, flag in zip(after_process_nouns, safe_flag_list):
                local_debug_output.append([n, "hallucination!" if flag else "safe!"])

            debug_output.append({**inference_data[i], "extract": local_debug_output})
            # Addition Ends
    ...
    if debug_output:
        save_structured_file(debug_output, debug_output_path)
